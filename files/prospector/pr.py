...
class ResumeProcessor:
    ...
    """
    COMMON_SKILLS = [
        "sales", "lead generation", "crm", "market research", "cold calling",
        "email marketing", "linkedin", "communication", "negotiation", "b2b sales",
        "customer relationship management", "data entry", "excel", "powerpoint",
        "presentation skills", "business development", "networking", "objection handling",
        "client acquisition", "account management", "public speaking", "teamwork",
        "problem solving", "critical thinking", "adaptability", "creativity",
        "hubspot", "salesforce", "outreach", "analytics", "reporting", "strategic planning",
        "relationship building", "persuasion", "time management", "organization"
    ]
    ...
    def __init__(self, csv_path, pdf_dir='downloaded_resumes', txt_dir='extracted_resumes'):
        ...
        logging.info(f"Initialized ResumeProcessor for prospector role with CSV: {self.csv_path}")
    ...
    def process_resumes(self):
        ...
        for idx, row in df.iterrows():
            ...
            if not resume_url or resume_url.lower() == 'unknown' or pd.isna(resume_url):
                logging.info(f"Skipping row {idx}: Invalid or unknown resume URL for prospector.")
                continue
            ...
            if not os.path.exists(pdf_path):
                logging.info(f"[{idx}] Downloading {pdf_name} for prospector role...")
            ...
            if not os.path.exists(txt_path):
                logging.info(f"[{idx}] Extracting text from {pdf_name} for prospector role...")
        ...
        logging.info("Resume processing for prospector role complete.")
        self.save_results()
...
class ResumeProcessorWithClustering(ResumeProcessor):
    ...
    def process_resumes(self):
        """
        Orchestrates resume processing, then performs clustering and analysis for prospector role.
        """
        super().process_resumes()

        if not self.extracted_features:
            logging.warning("No extracted features available for clustering for prospector role.")
            return
        ...
        if not text_data:
            logging.error("No resume texts found for clustering after extraction for prospector role.")
            return
        ...
        try:
            ...
            logging.info("Merged 'final hiring status' from original CSV for prospector role.")
        except Exception as e:
    ...
    def _cluster_and_analyze(self, n_clusters=5):
        """
        Performs K-Means clustering on resume texts and analyzes the clusters for prospector role.
        """
        logging.info(f"Performing K-Means clustering with {n_clusters} clusters for prospector role...")
        ...
        try:
            ...
            logging.info(f"Clustering complete. {n_clusters} groups formed for prospector role.")
        except Exception as e:
            logging.error(f"K-Means clustering failed for prospector role: {e}")
            return
    ...
    def generate_cluster_summary(self, filename="cluster_summary.txt"):
        ...
        with open(filename, "w", encoding='utf-8') as f:
            f.write("Prospector Role Resume Clustering Summary\n")
            f.write("=" * 30 + "\n\n")
            for cluster_id in sorted(self.df_full['cluster'].unique()):
                cluster_df = self.df_full[self.df_full['cluster'] == cluster_id]
                success_df = cluster_df[cluster_df['final hiring status'].astype(str).str.lower().str.contains("hired", na=False)]
    ...
    def plot_clusters(self, method='pca', filename="cluster_plot.png"):
        ...
        logging.info(f"Generating cluster plot using {method.upper()} for prospector role...")
        ...
        try:
            ...
            plt.title(f"Prospector Resume Clustering ({method.upper()})", fontsize=16)
            ...
            logging.error(f"Error generating {method.upper()} plot for prospector role: {e}")
...
if __name__ == '__main__':
    ...
    if not os.path.exists('../cleaned'):
        logging.warning("Directory 'cleaned' not found. Please ensure 'preprocessed_prospector_role.csv' is in the correct path.")

    # Initialize and run the processor with clustering capabilities
    processor = ResumeProcessorWithClustering(
        csv_path='../cleaned/preprocessed_prospector_data_cleaned.csv',
        pdf_dir='../../prospector_resumes/downloaded_resumes',
        txt_dir='../../prospector_resumes/extracted_resumes'
    )
    ...
    os.makedirs('prospector_out', exist_ok=True)  # Ensure output directory exists
    processor.export_clusters_to_csv("prospector_out/clustered_resumes_{timestamp}.csv")
    processor.generate_cluster_summary("prospector_out/cluster_summary_{timestamp}.txt")
    processor.plot_clusters(method='pca', filename="prospector_out/resume_clusters_pca_{timestamp}.png")
    processor.plot_clusters(method='tsne', filename="prospector_out/resume_clusters_tsne_{timestamp}.png")