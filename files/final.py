...
class ResumeProcessor:
    ...
    def _download_pdf(self, file_id, output_path):
        ...
        try:
            gdown.download(url, output_path, quiet=True, fuzzy=True) # Added fuzzy=True for more robust download
    ...
    def _extract_experience_years(self, text):
        ...
        patterns = [
            ...
            r"(\d+)\s*yrs?\s*(?:exp)?",
            r"(\d+)\s*-\s*(\d+)\s*years?\s+experience" # e.g., "3-5 years experience"
        ]
        found = []
        for pattern in patterns:
            for match in re.finditer(pattern, text, re.IGNORECASE):
                if '-' in match.group(0): # Handle ranges like "3-5 years" by taking the upper bound
        ...
        for idx, row in df.iterrows():
            ...
            if text:
                ...
                self.extracted_features.append({
                    ...
                    "Resume_Text_File": txt_name # Add reference to the extracted text file
                })
            else:
...
class ResumeProcessorWithClustering(ResumeProcessor):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.df_full = None  # DataFrame including all metadata + clustering results
    ...
    def process_resumes(self):
        ...
        """
        super().process_resumes() # Run the base processing first
        ...
        try:
            ...
            # Ensure 'Applicant' column is string type for merging
            ...
            self.df_full['final hiring status'] = "Unknown" # Default if merge fails
    ...
    def _cluster_and_analyze(self, n_clusters=5):
        ...
        self.vectorizer = TfidfVectorizer(stop_words='english', max_features=2000, ngram_range=(1,2)) # Increased max_features and added ngram_range
        ...
        # K-Means Clustering
        try:
            model = KMeans(n_clusters=n_clusters, random_state=42, n_init=10) # n_init is important for robust results
        ...
        for cluster_id in sorted(self.df_full['cluster'].unique()):
            ...
            # Handle potential NaN values in 'final hiring status' before lowercasing
            success_df = cluster_df[cluster_df['final hiring status'].astype(str).str.lower().str.contains("hired", na=False)]
            # Use .astype(str) to avoid errors with non-string types if 'final hiring status' has NaNs or other types
    ...
    def _top_keywords(self, texts, vectorizer, X_subset):
        ...
        top_indices = cluster_tfidf.argsort()[::-1][:10] # Get top 10 keywords
    ...
    def plot_clusters(self, method='pca', filename="cluster_plot.png"):
        ...
        if method == 'pca':
            ...
            # Adjust perplexity if dataset is small. Typical values are 5 to 50.
            # If you have very few samples, you might need to lower it.
            n_samples = self.X.shape[0]
            perplexity_val = min(30, max(5, n_samples - 1)) if n_samples > 1 else 1 # Ensure perplexity is valid
            reducer = TSNE(n_components=2, random_state=42, perplexity=perplexity_val, n_jobs=-1) # Added n_jobs for faster computation
        else:
        ...
        try:
            ...
            sns.scatterplot(
                ...
                hue='cluster',
                palette='tab10',
                style=self.df_full['final hiring status'].astype(str).str.contains("hired", case=False, na=False), # Ensure 'final hiring status' is string
                markers={True: 'o', False: 'X'}, # Explicitly define markers for hired/not hired
            ...
            plt.legend(title='Cluster / Hired Status', bbox_to_anchor=(1.05, 1), loc='upper left') # Move legend outside
            plt.grid(True, linestyle='--', alpha=0.6) # Add grid
            plt.tight_layout(rect=[0, 0, 0.85, 1]) # Adjust layout to prevent legend overlap
            plt.savefig(filename, dpi=300) # Save with higher resolution
            # plt.show() # Only show if running in an interactive environment
...
if __name__ == '__main__':
    ...
    if not os.path.exists('cleaned'):
        logging.warning("Directory 'cleaned' not found. Please ensure 'preprocessed_outreacher_role.csv' is in the correct path.")
        # You might want to create it or handle this case based on your project structure

    if not os.path.exists('cleaned/preprocessed_outreacher_role.csv'):
        # Create a dummy CSV for demonstration if it doesn't exist.
        # In a real scenario, the user would provide this file.
        os.makedirs('cleaned', exist_ok=True)
        dummy_data = {
            'Applicant': ['John Doe', 'Jane Smith', 'Peter Jones', 'Alice Brown', 'Bob White'],
            'Resume': [
                'https://drive.google.com/file/d/1Xy_Z_ABC_123/view?usp=sharing',
                'https://drive.google.com/file/d/2Ab_C_DEF_456/view?usp=sharing',
                'https://drive.google.com/file/d/3Cd_E_FGH_789/view?usp=sharing',
                'https://drive.google.com/file/d/4De_F_GHI_012/view?usp=sharing',
                'https://drive.google.com/file/d/5Ef_G_HIJ_345/view?usp=sharing'
            ],
            'final hiring status': ['Hired', 'Not Hired', 'Hired', 'Not Hired', 'Hired']
        }
        dummy_df = pd.DataFrame(dummy_data)
        dummy_df.to_csv('cleaned/preprocessed_outreacher_role.csv', index=False)
        logging.info("Created a dummy 'preprocessed_outreacher_role.csv' for demonstration.")
    # Initialize and run the processor with clustering capabilities
    processor = ResumeProcessorWithClustering(
        csv_path='cleaned/preprocessed_outreacher_role.csv',
        pdf_dir='downloaded_resumes',
        txt_dir='extracted_resumes'
    )
    ...
    processor.export_clusters_to_csv("clustered_resumes.csv")
    processor.generate_cluster_summary("cluster_summary.txt")
    processor.plot_clusters(method='pca', filename="resume_clusters_pca.png")
    processor.plot_clusters(method='tsne', filename="resume_clusters_tsne.png")